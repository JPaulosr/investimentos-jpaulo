# pages/2_💰_Proventos.py — página Proventos (versão turbinada)
...
with st.sidebar:
    ...
    if PV.empty or "Data" not in PV.columns:
        ...
        # por padrão, últimos 18 meses para dar contexto
        default_ini = max(dmin, dmax - timedelta(days=540))
...
# média mensal do período filtrado (considerando meses com zero)
...
# progresso versus meta (média dos últimos 3 meses)
...
# -------------------- Gráfico: barras + acumulado e eixo mensal contínuo --------------------
fig = go.Figure()
...
tabela_cal.columns = [pd.to_datetime(f"2000-{m}-01").strftime("%b") for m in tabela_cal.columns]  # nomes curtos
st.dataframe(tabela_cal, use_container_width=True)

# Boxplot por mês (mostra variação histórica)
if not PVm.empty:
    pv_box = PVm.groupby([PVm["Data"].dt.month, PVm["Data"].dt.year])["Valor"].sum().reset_index()
    pv_box.columns = ["Mes","Ano","Valor"]
    pv_box["MesNome"] = pv_box["Mes"].apply(lambda m: pd.to_datetime(f"2000-{m}-01").strftime("%b"))
...
# -------------------- Rodapé --------------------
st.caption(f"Modo de leitura: **{st.session_state.get('proventos_modo','desconhecido')}** | "
          f"Melhor mês do período: **{melhor_mes['Competencia'].strftime('%b/%Y')}** = {moeda_br(melhor_mes['Valor'])}")
